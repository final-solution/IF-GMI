...
def main():
    ...
    # gpu_devices = [i for i in range(torch.cuda.device_count()-1)]
    ...
    if config.logging:
        Path("results/PPA").mkdir(parents=True, exist_ok=True)
        init_w_path = f"results/PPA/init_w_{run_id}.pt"
    ...
    if config.logging:
        optimized_w_path = f"results/PPA/optimized_w_{run_id}.pt"
    ...
    if config.logging:
        optimized_w_path_selected = f"results/PPA/optimized_w_selected_{run_id}.pt"
    ...
    try:
        ...
        if config.logging:
            try:
                filename_precision = write_precision_list(
                    f'results/PPA/precision_list_unfiltered_{run_id}',
                    precision_list)
        ...
        if config.final_selection:
            ...
            if config.logging:
                filename_precision = write_precision_list(
                    f'results/PPA/precision_list_filtered_{run_id}',
                    precision_list)
                wandb.save(filename_precision)
    ...
    try:
        ...
        if config.logging:
            try:
                filename_distance = write_precision_list(
                    f'results/PPA/distance_inceptionv3_list_filtered_{run_id}',
                    mean_distances_list)
        ...
        if target_dataset in [
            ...
            if config.logging:
                filename_distance = write_precision_list(
                    f'results/PPA/distance_facenet_list_filtered_{run_id}',
                    mean_distances_list)
                wandb.save(filename_distance)
